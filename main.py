...
class Game(Funcionalidades):
    ...
    def menu_config(self) -> None:
        ...
        self.btn_play = Button((LARGURA // 2, ALTURA // 2 + 80), os.path.join(DIRETORIO_IMAGENS, 'Menu/btn_play_normal.png').replace('\\', '/'), os.path.join(DIRETORIO_IMAGENS, 'Menu/btn_play_hover.png').replace('\\', '/'), self.play)
        self.btn_controls = Button((LARGURA // 2, ALTURA // 2 + 160), os.path.join(DIRETORIO_IMAGENS, 'Menu/btn_controls_normal.png').replace('\\', '/'), os.path.join(DIRETORIO_IMAGENS, 'Menu/btn_controls_hover.png').replace('\\', '/'), self.to_controls)

    def controls_config(self) -> None:
        '''Configura a tela de controles.'''
        self.teclado_contros_img = pygame.image.load(os.path.join(DIRETORIO_IMAGENS, 'Controls/teclado_controls.png').replace('\\', '/'))
    ...
    def menu(self) -> None:
        '''início do jogo.'''
        self.screen.blit(self.title, self.title.get_rect(center=((LARGURA // 2, ALTURA // 2))))
                ...
                case 'JOGO':
                    ...
                    if self.level.fim:
                        if os.path.isdir(os.path.join(DIRETORIO_MAPAS, f'Mapa {self.mapa + 1}').replace('\\', '/')):
                            ...
                            self.btn_menu_extenso_voltar.draw(self.screen)
            pygame.display.flip()
